...
Bench file: `kayangan-bench.html` (v5.2, ~1150 lines) — Step 5 rim waterfalls, 2nd aesthetic pass (§14b: drifting emission field breaks the ring + boot pre-distribution + per-particle floor; on top of §14a clustered/fanned sheets), WebGPU compute + WebGL2 CPU fallback, over Step 4 water + §12 refraction fix + panel toggle. Log + bench both in outputs / user's GitHub.
---
