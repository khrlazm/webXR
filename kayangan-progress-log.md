...
## 15b. Step 6 — DEVICE TEST 2 + FINAL DIRECTION (July 21, 2026)

Second on-device pass. Fog rotation fixed (reads correctly now). Three directional calls from the user, applied:

1. **God rays removed** — cut entirely (node, uniforms, `updateSunScreen` CPU projection, UI checkbox + slider, render-loop call). The effect was borderline for this scene (dark disc, low sun) and not worth the fill cost / tuning. The post graph is simpler for it: `sceneColor + bloom → renderOutput → graded → [fxaa]`.
2. **"Punchy" colour grade added** — a display-space grade node after `renderOutput()` (i.e. after AgX): saturation + contrast-around-mid + slight gain, driven by ONE `uPunch` uniform (0 = neutral → punchy). Default **0.75** (sat +~28%, contrast +~18%). Cheap ALU, so it stays on **even in the Quest preset** — it's the look, not an extra. Live "Punch (grade)" slider, no graph rebuild. Bold, saturated read to match the mystical concept art.
3. **Mystical music added** — an **original generative *tintinnabuli* engine** (pure Web Audio, no deps, no external audio files; reverb IR synthesised). Tintinnabuli is Arvo Pärt's *technique* — a stepwise M-voice shadowed by a T-voice that only sounds tonic-triad notes — **not** a specific composition, so nothing copyrighted is reproduced. A-minor, root+fifth drone, bell-like triangle/sine voices, slow overlapping phrases, long hall reverb, gentle fade-in. Toggled by the "Music (tintinnabuli)" checkbox (which supplies the gesture Web Audio needs). Slow meditative wander, effectively endless, low CPU alongside WebGPU.

**Quest preset** relabelled "lean post": tone-map + bloom + grade, FXAA off, rim + fog gated. **Bench now v6.2.**

**Per the user, the bench is DONE — Step 7 (formal exit test) is waived.** The vertical-slice visual target (Steps 0–6) is met; remaining items below are optional polish, not blockers.

## 16. Handoff / optional follow-ups (bench considered complete)
...
2. **Confirm Step 6 look on A06 + Quest 2** (optional, since the target's met on desktop): (a) **sun glow + waterfall sheets bloom**, lit snow/terrain does NOT wash out (if it does → raise threshold, or go MRT selective); (b) **punch grade** reads bold but not clipped — back `uPunch` off if highlights crush; (c) **rim glow** reads as a thin atmospheric limb, doesn't over-glow the top; (d) **under-disc fog** sits as a flat halo, no visible quad edge from low angles; (e) **Quest-preset A/B** — fps lean-post vs full stack at max water config. (f) **Music** — confirm the tintinnabuli engine starts on the checkbox and doesn't fight WebGPU for CPU on Quest.
3. **Step 4.3 (§13) — night water "lifted"**, optional fold-in: gate env reflection + `deepCol` by `uNight`/`horizon`. Deferred, not blocking.
4. **Phase-1 lift** — which bench modules graduate into the Vite/TS scaffold: procedural terrain, water + refraction, waterfalls, the post stack (bloom + grade), and the music engine are all self-contained enough to port.

**Risk register (final):** #1 (depth parity → water) **RETIRED**. #2 (TSL compute on WebGL2 → waterfalls) — built + API-verified, **verify on-device**. #4 (bloom cost on Quest) — bloom is baseline/always-on; Quest preset is the measurement lever. God-ray fill-cost risk **removed with the effect**. Mitigations staged: threshold-only bloom (no MRT), FXAA off on low, grade is ALU-cheap, §11 resolution-governor still on the shelf.

Bench file: `kayangan-bench.html` (v6.2, ~1489 lines) — **Step 6 final**: threshold bloom + **"punchy" display-space grade** + fresnel rim glow + flat under-disc fog + **generative tintinnabuli music** + Quest-preset (lean post) A/B. God rays removed. All visual post through the §12 pass (`outputColorTransform=false` → single `renderOutput()` tone-map → grade → display-space FXAA); scene pass always-on, direct path retired. Over Step 5 waterfalls (§14a–c) + Step 4 water + §12 refraction fix. **Bench complete (Steps 0–6); Step 7 exit test waived by user.** Log + bench both in outputs / user's GitHub.
---
